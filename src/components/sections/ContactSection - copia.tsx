...
/**
 * Data shapes for the Contact section.
 *
 * Copy is fed by props (ready for CMS/i18n). Submission is delegated to an
 * `onSubmit` callback so the page owner decides how to deliver the message
 * (server action, API route, etc.).
 */
export interface ContactInfo {
  email: string;
  phone: string;
  /** Address rendered as one or more lines. */
  addressLines: string[];
}
...
export interface ContactSectionProps {
  ...
  /** Called with the form values once required fields and consent are valid. */
  onSubmit?: (values: ContactFormValues) => void | Promise<void>;
  /** Optional decorative artwork (the UFO), shown bottom-left on large screens. */
...
/**
 * Contact section: contact details on the left, an accessible message form on
 * the right. Client component — owns the form state and validation.
 */
export default function ContactSection({
  ...
  const { accent, labels, info } = content;
  ...
  return (
    ...
    >
      ...
      <div className='relative container-site py-16 sm:py-20 lg:py-24'>
         {decoration && (
        <div className='pointer-events-none absolute bottom-6 -left-95 hidden w-110 3xl:block -rotate-6'>
          <Image
            priority
            src={decoration.src}
            alt={decoration.alt ?? ''}
        ...
        <div className='grid gap-12 lg:grid-cols-2 lg:gap-16 px-8 lg:px-0'>
          {/* Left — details */}
